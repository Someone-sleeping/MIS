...
class InteractiveSegmentationGUI:
    def __init__(self, segmentation_model):
        ...
        self.app.initialize()
        self.window = self.app.create_window("Interactive4D", 1400, 700)
    ...
    def __key_event(self, event):
        ...
        if event.type == gui.KeyEvent.DOWN and key_mapper.get(event.key) != None:
            if key_mapper.get(event.key) == "0":
                self.cur_obj_idx, self.cur_obj_key = "10", "10"
            else:
                self.cur_obj_idx = key_mapper.get(event.key)
                self.cur_obj_key = key_mapper.get(event.key)
        ...
        if event.key == 10:  # "Enter" key pressed
            self.__run_segmentation()
            return gui.Widget.EventCallbackResult.HANDLED
        elif event.key == 96:  # "~" pressed for toggling Original/Semantics colors
    ...
    def __mouse_event(self, event):
        ...
        if event.type == gui.MouseEvent.WHEEL:
            ...
            if self.material_record.point_size <= self.window.scaling * 2.5 and event.wheel_dy >= 0:
                ...
                self.material_record.point_size -= 0.7 * event.wheel_dy
    ...
    def __set_textfield(self, new_object_name):
        ...
        gui.Application.instance.post_to_main_thread(self.window, self.__update_textfield)
    ...
    def __update_object(self):
        ...
        self.objects_widget.toggle_info.text = f"Currently segmenting object '{self.cur_obj_name}'.\nSelect a different object:"
    ...
    def set_new_scene(self, scene_name, point_object, coords, coords_qv, colors, original_colors, original_labels, original_labels_qv, is_point_cloud=True, object_names=[], gt_masks_colors=None):
        ...
        # post to main thread
        gui.Application.instance.post_to_main_thread(self.window, self.__update_scene)
    ...
    def select_object(self, colors=None):
        ...
        if colors is not None:
            ...
            gui.Application.instance.post_to_main_thread(self.window, self.__update_colors)
        self.objects_widget.update_buttons()
        gui.Application.instance.post_to_main_thread(self.window, self.__update_object)
    ...
    def __init_points(self, point_object, coords, coords_qv, colors, original_colors, original_labels, original_labels_qv, is_point_cloud, gt_masks_colors):
        ...
        if self.original_labels is not None:
            ...
            self.new_labels = self.original_labels
        else:
...
class UserInstruction(gui.CollapsableVert):
    ...
    def __init__(self, spacing, margin, font, separation_height):
        ...
        descr_obj = gui.Label("{: <30}Object".format("[key + Click]"))
        ...
        desr_toggle_colors = gui.Label("{: <30}Toggle Color Mode".format("[~]"))
...
class Objects(gui.CollapsableVert):
    ...
    def update_buttons(self):
        ...
        for object_name, object_idx in self.objects.items():
            ...
            butt.background_color = gui.Color(*get_obj_color(object_idx, normalize=True))
    ...
    def create_object(self, object_name=None, load_colors=False, cur_obj_key=None):
        ...
        self.current_object_idx = self.objects[object_name]
